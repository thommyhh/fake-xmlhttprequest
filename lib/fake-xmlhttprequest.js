                    ...
                    var handleResponse = function() {
                        ...
                        if (typeof item.response === 'string') {
                            ...
                            if (item.url instanceof RegExp) {
                                request.response = item.response.apply(null, item.url.exec(request.url));
                            } else {
                                request.response = item.response(request.url);
                            }
                            if (request.responseType === '') {
                            ...
                            if (typeof item.proxy === 'string') {
                                ...
                                if (item.url instanceof RegExp) {
                                    url = item.proxy.apply(null, item.url.exec(request.url));
                                } else {
                                    url = item.proxy(request.url);
                                }
                            }
                            if (typeof url === 'string' && url.length > 0) {
                                ...
                                proxyRequest.onreadystatechange = function() {
                                    if (proxyRequest.readyState === XMLHttpRequest.DONE && proxyRequest.status === 200) {
                                        ...
                                        request.readyState = XMLHttpRequest.DONE;
                                        if (request.responseType === '') {
                                            request.responseText = proxyRequest.responseText;
                                            request.responseXML = proxyRequest.responseXML;
                                        }
                                        request.readyState = XMLHttpRequest.DONE;
                                        if (typeof request.onreadystatechange === 'function') {
                                            request.onreadystatechange();
